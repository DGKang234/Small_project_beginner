...
import vpython
#https://codereview.stackexchange.com/questions/244714/blackjack-with-card-representation-vpython
#https://github.com/zsonnen/blackjack/blob/master/blackjack.py
"""
BLACKJACK GAME:
vpython file: numerous pretty ways to display cards
"""

def clear():
    sp.run(('cls' if os.name == 'nt' else 'clear'), shell=True)     # depends on MS window version choese 'cls' or 'clean'
...
Card = collections.namedtuple('Card', ['value', 'suit'])            # factory function for creating tuple subclasses with named fields
...
class Deck:
    ...
    def deck_visual(self):
        s = [vpython.tiny_card_visual(card) for card in self.cards if card.suit == 'Spades']
        d = [vpython.tiny_card_visual(card) for card in self.cards if card.suit == 'Diamonds']
        h = [vpython.tiny_card_visual(card) for card in self.cards if card.suit == 'Hearts']
        c = [vpython.tiny_card_visual(card) for card in self.cards if card.suit == 'Clubs']
        vpython.print_cards(s)
        vpython.print_cards(d)
        vpython.print_cards(h)
        vpython.print_cards(c)
...
class Hand:
    ...
    def card_visual(self):
        card_list = [vpython.reg_card_visual(card) for card in self.hand]
        vpython.print_cards(card_list)
        print(f"\nTotal of: {self.hand_score()}\n")

    def mini_card_visual(self):
        card_list = [vpython.mini_card_visual(card) for card in self.hand]
        vpython.print_cards(card_list)
        print(f"\nTotal of: {self.hand_score()}\n")
...
class Dealer(Hand):
    ...
    def dealer_visual(self):
        hidden_card = vpython.reg_hidden_card
        card_list = [hidden_card] + [vpython.reg_card_visual(card) for card in self.hand[1:]]
        vpython.print_cards(card_list)
